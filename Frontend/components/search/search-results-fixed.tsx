    ...
    <div className="space-y-6">
      ...
      {playlists.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="flex space-x-4 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {playlists.slice(0, 8).map((playlist: any, index: number) => (
              <div
              ...
              >
                <div className="relative">
                  <Avatar className="w-full h-32 rounded-lg mb-3">
                    <AvatarImage 
                      src={getImageWithFallback(playlist, 'playlist')} 
                      alt={playlist.name}
                      className="object-cover rounded-lg"
                    />
                    <AvatarFallback className="rounded-lg bg-muted">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    By {playlist.owner?.name || playlist.creator || 'User'}
                  </p>
                </div>
              </div>
            ))}
      ...
      {users.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="flex space-x-4 overflow-x-auto pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {users.slice(0, 8).map((user: any, index: number) => (
              <div
                key={user.id || index}
                className="flex-shrink-0 cursor-pointer group"
              >
                <div className="relative">
                  <Avatar className="w-32 h-32 border-4 border-transparent group-hover:border-white/20 transition-colors">
                    <AvatarImage 
                      src={getImageWithFallback(user, 'user')} 
